...
//    "-1.1E2"                    //49
    ...
    for(; tests[i][0] != '\0'; i++ ){
        ...
        switch(i){
            ...
            case 28:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 0      ? "Pass" : "Fail" ); break;
            case 29:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 1      ? "Pass" : "Fail" ); break;
            case 30:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 10     ? "Pass" : "Fail" ); break;
            case 31:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 11     ? "Pass" : "Fail" ); break;
            case 32:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 0      ? "Pass" : "Fail" ); break;
            case 33:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 0      ? "Pass" : "Fail" ); break;
            case 34:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 1000   ? "Pass" : "Fail" ); break;
            case 35:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 1024   ? "Pass" : "Fail" ); break;
            case 36:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_DOUBLE && num.val_dble == 1.024 * 1000 * 1000          ? "Pass" : "Fail" ); break;
            case 37:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 1024 * 1024                  ? "Pass" : "Fail" ); break;
            case 38:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_DOUBLE && num.val_dble == 3.6 * 1000 * 1000 * 1000     ? "Pass" : "Fail" ); break;
            case 39:num = parse_number(tests[i],0); printf("%s\n", num.type == CAMIO_UINT64 && num.val_uint == 2 * 1024 * 1024 * 1024ULL    ? "Pass" : "Fail" ); break;
            default: printf("Fail\n"); break;
        }
    }
}
