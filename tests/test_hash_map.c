...
static ch_word test1_i64(kv* test_data)
{
    ...
    ch_hash_map* hm1 = ch_hash_map_new(10,sizeof(void*),cmp_i64, 0);
    hash_map_delete(hm1);
...
static ch_word test2_i64(kv* test_data)
{
    ...
    ch_hash_map_it it1 = hash_map_push_unsafe(hm1,test_data[0].key,strlen(test_data[0].key), &test_data[0].value );
...
static ch_word test3_i64(kv* test_data)
{
    ...
    for(ch_word i = 0; i < 3; i++){
        ...
        ch_hash_map_it it1 = hash_map_push_unsafe(hm1,test_data[i].key,strlen(test_data[i].key), &test_data[i].value );
...
static ch_word test4_i64(kv* test_data)
{
    ...
    //Make an empty hash_map
    ch_hash_map* hm1 = ch_hash_map_new(10,sizeof(void*),cmp_i64, 1);
    hash_map_delete(hm1);
...
static ch_word test5_i64(kv* test_data)
{
    ...
    ch_hash_map_it it1 = hash_map_push_unsafe(hm1,&key,sizeof(key), &test_data[0].value );

    CH_ASSERT(it1.key == &key);
...
static ch_word test6_i64(kv* test_data)
{
    ...
    for(ch_word i = 0; i < 3; i++){
        ...
        ch_hash_map_it it1 = hash_map_push_unsafe(hm1,&key,sizeof(key), &test_data[i].value );
        ch_hash_map_it it2 = hash_map_get_first(hm1,test_data[i].key, strlen(test_data[i].key) );
        ch_hash_map_it tmp;
        while( (tmp = hash_map_get_next(it2)).value ){
            it2 = tmp;
